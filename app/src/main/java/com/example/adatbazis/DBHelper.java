...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String COL_VEZNEV = "vezetéknév";
    private static final String COL_KERNEV = "keresztnév";
    ...
    public boolean modositas(String id, String vezNev, String kerNev, String jegy){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(COL_VEZNEV, vezNev);
        values.put(COL_KERNEV, kerNev);
        values.put(COL_JEGY, jegy);
        int result = db.update(TABLE_NAME, values,COL_ID + " =?",
                new String[]{id});
        return result > 0;
    }
}
